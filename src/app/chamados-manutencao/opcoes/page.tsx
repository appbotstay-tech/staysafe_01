...
export default async function ChamadosManutencaoOpcoesPage({
  searchParams
}: PageProps) {
        ...
        <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
          <div>
            <h1 className="text-2xl font-semibold text-slate-900 dark:text-slate-100">
              Gerenciar Opções - Chamados de Manutenção
            </h1>
            <p className="mt-2 text-sm text-slate-600 dark:text-slate-300">
              Configure orientações exibidas no módulo de chamados.
            </p>
          </div>
        ...
        <section
          className={`rounded-xl border p-4 text-sm ${
            feedbackType === "error"
              ...
              : "border-emerald-200 bg-emerald-50 text-emerald-700 dark:border-emerald-800 dark:text-emerald-200"
          }`}
        >
          {feedback}
        </section>
